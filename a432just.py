...
def main():
    voices = 3
    s = Sequence(fundamental=432, equal=False, voices=voices)
    notes = list("abcdefg")
    ...
    while song_duration_secs > 0:
        ...
        s.add_note(notes_to_harmonies[note]["5"], duration, random.randint(-2, 1), voice=1)
        s.add_note(notes_to_harmonies[note]["3"], duration, random.randint(-2, 1), voice=2)

    s.write_file("a432just.wav")
